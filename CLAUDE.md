...
The impact section reports at three altitudes, because they answer different questions: totals per network (**where** the campaign worked), each influencer ranked by views with a per-platform breakdown (**who** delivered, and from which account), and the evolution curve. The same creator usually posts on two networks and those numbers are not comparable to each other — a Reel and a TikTok video are not measured the same way — so they are summed to rank people but shown apart to explain the ranking.
